...
export default function ProductsPage() {
  ...
  async function load() {
    setLoading(true)
    const url = businessFilter ? `/api/products?businessId=${businessFilter}` : '/api/products'
  ...
  const filtered = products.filter(p => {
    ...
    return !q || p.name.toLowerCase().includes(q) || p.business?.name.toLowerCase().includes(q)
  })
  ...
  async function handleCreate(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!data.businessId || !data.name || isNaN(data.price)) return
  ...
  return (
    <div style={{ padding: '32px 36px', background: C.bgAlt, minHeight: '100vh' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: C.ink, margin: 0 }}>Produits</h1>
        <button
      ...
      {creating && (
        <div style={{
          ...
          <h3 style={{ fontSize: 16, fontWeight: 600, color: C.ink, marginBottom: 16 }}>Créer un produit</h3>
      ...
      {editing && (
        <div style={{
          ...
          <h3 style={{ fontSize: 16, fontWeight: 600, color: C.ink, marginBottom: 16 }}>Modifier le produit</h3>
          <form onSubmit={handleUpdate}>
            <input name="name" defaultValue={editing.name} placeholder="Nom du produit" required
              style={{ width: '100%', padding: '8px 12px', borderRadius: 6, border: `1px solid ${C.border}`, fontSize: 13, marginBottom: 12 }} />
      ...
      {loading ? (
        ...
        <div style={{ padding: 40, textAlign: 'center', color: C.mid }}>Aucun produit trouvé</div>
      ) : (
        <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 12, overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: C.bgAlt, borderBottom: `1px solid ${C.border}` }}>
                {['Nom', 'Business', 'Prix', 'Disponible', 'Actions'].map(h => (
            ...
            <tbody>
              {filtered.map((p, i) => (
                <tr key={p.id} style={{ borderBottom: i < filtered.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                  <td style={{ padding: '10px 14px', fontWeight: 500, color: C.ink }}>{p.name}</td>
                  <td style={{ padding: '10px 14px', color: C.mid }}>{p.business?.name || '—'}</td>
                  <td style={{ padding: '10px 14px', color: C.ink }}>{p.price} DT</td>
                  <td style={{ padding: '10px 14px' }}>
                    <span style={{ background: p.available ? C.greenBg : C.redBg, color: p.available ? C.green : C.red, borderRadius: 4, padding: '2px 8px', fontSize: 11, fontWeight: 600 }}>
                      {p.available ? 'Oui' : 'Non'}
                    </span>
                  </td>
                  <td style={{ padding: '10px 14px' }}>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={() => setEditing(p)} style={{ padding: 4, background: 'none', border: 'none', cursor: 'pointer', color: C.mid }}>
                        <Edit size={14} />
                      </button>
                      <button onClick={() => handleDelete(p.id)} style={{ padding: 4, background: 'none', border: 'none', cursor: 'pointer', color: C.red }}>
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
