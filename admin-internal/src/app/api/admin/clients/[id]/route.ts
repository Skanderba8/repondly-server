// admin-internal/src/app/api/clients/[id]/route.ts
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.error(">>> GET /api/admin/clients/[id] CRASHED:", error)
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    for (const key of allowedFields) {
      if (key in body) {
        // Ensure accountId is properly cast as a Number for Prisma
        if (key === 'chatwootAccountId' && body[key] !== null) {
    ...
    console.error(">>> PATCH /api/admin/clients/[id] CRASHED:", error)
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 })
  }
}