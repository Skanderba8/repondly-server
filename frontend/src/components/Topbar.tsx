...
export default function Topbar({ session }: { session: Session }) {
  ...
  return (
    <header style={{
      height: '56px', background: '#ffffff', borderBottom: '1px solid #ebebea',
      display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      padding: '0 32px', flexShrink: 0, position: 'sticky', top: 0, zIndex: 10,
    }}>
      {/* Page title injected via CSS — left side empty for now */}
      <div />

      {/* Right side */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', position: 'relative' }}>
        {/* Chatwoot link */}
        <a href="https://app.repondly.com" target="_blank" style={{
          display: 'flex', alignItems: 'center', gap: '6px',
          fontSize: '0.8rem', color: '#6b6b67', textDecoration: 'none',
          padding: '6px 12px', borderRadius: '8px', border: '1px solid #ebebea',
        ...
        <div style={{ position: 'relative' }}>
          ...
          {menuOpen && (
            <div style={{
              ...
              background: '#ffffff', border: '1px solid #ebebea',
              ...
              <button onClick={() => signOut({ callbackUrl: '/auth/signin' })} style={{
                ...
                fontSize: '0.875rem', color: '#dc2626', borderRadius: '6px',
