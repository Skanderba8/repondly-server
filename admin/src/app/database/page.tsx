...
import {
  ...
  Activity, Table2, GitBranch, Play, HardDrive, Search, Trash2, X, Eye,
...
function TableViewerModal({ tableName, isSuperAdmin, onClose }: { tableName: string; isSuperAdmin: boolean; onClose: () => void }) {
  ...
  useEffect(() => { setPage(1); void fetchData(1) }, [search])
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(13,27,46,0.5)', zIndex: 100, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '40px 20px', overflowY: 'auto' }}>
      <div style={{ background: C.bg, borderRadius: 16, width: '100%', maxWidth: 1100, boxShadow: '0 24px 80px rgba(0,0,0,0.2)', overflow: 'hidden' }}>
        <div style={{ padding: '16px 22px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: C.bgAlt }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, border: `1px solid ${C.border}`, borderRadius: 7, padding: '6px 10px', background: C.bg }}>
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Filtrer…" style={{ border: 'none', outline: 'none', fontSize: 12, color: C.ink, background: 'transparent', width: 120 }} />
            </div>
        ...
        <div style={{ overflowX: 'auto', maxHeight: '60vh', overflowY: 'auto' }}>
          {loading ? (
            <div style={{ padding: 40, textAlign: 'center' }}>
              <div style={{ width: 24, height: 24, border: `2px solid ${C.border}`, borderTopColor: C.blue, borderRadius: '50%', margin: '0 auto' }} />
            </div>
          ) : error ? (
                  ...
                  <tr key={i} style={{ borderBottom: `1px solid ${C.border}`, transition: 'background 0.1s' }}
                    ...
                    {isSuperAdmin && (
                      <td style={{ padding: '8px 12px' }}>
                        <button onClick={() => setDeleteConfirm(row)} style={{ background: 'none', border: `1px solid ${C.border}`, borderRadius: 5, padding: '3px 6px', cursor: 'pointer', color: C.mid, display: 'flex', alignItems: 'center', transition: 'all 0.1s' }}
        ...
        {data && data.pages > 1 && (
          <div style={{ padding: '12px 22px', borderTop: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: C.bgAlt }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} style={{ border: `1px solid ${C.border}`, borderRadius: 6, padding: '5px 9px', background: C.bg, cursor: page === 1 ? 'not-allowed' : 'pointer', opacity: page === 1 ? 0.4 : 1, color: C.mid, display: 'flex', alignItems: 'center' }}>
                <ChevronLeft size={13} />
              </button>
              <button onClick={() => setPage(p => Math.min(data.pages, p + 1))} disabled={page === data.pages} style={{ border: `1px solid ${C.border}`, borderRadius: 6, padding: '5px 9px', background: C.bg, cursor: page === data.pages ? 'not-allowed' : 'pointer', opacity: page === data.pages ? 0.4 : 1, color: C.mid, display: 'flex', alignItems: 'center' }}>
      ...
      {deleteConfirm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(13,27,46,0.6)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: C.bg, borderRadius: 12, padding: '24px 28px', maxWidth: 380, width: '90%', boxShadow: '0 16px 48px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => setDeleteConfirm(null)} style={{ padding: '7px 14px', border: `1px solid ${C.border}`, borderRadius: 7, background: C.bg, color: C.mid, cursor: 'pointer', fontSize: 13 }}>Annuler</button>
              <button onClick={() => void handleDelete(deleteConfirm)} disabled={!!deleting} style={{ padding: '7px 14px', border: 'none', borderRadius: 7, background: C.red, color: '#fff', cursor: 'pointer', fontSize: 13, fontWeight: 600, opacity: deleting ? 0.6 : 1 }}>
...
export default function DatabaseManager() {
  ...
  return (
    <div style={{ padding: '32px 36px', background: C.bgAlt, minHeight: '100vh' }}>
      ...
      {migrateResult && (
        <div style={{ marginBottom: 18, background: migrateResult.success ? C.greenBg : C.redBg, border: `1px solid ${migrateResult.success ? C.green : C.red}30`, borderRadius: 12, padding: '14px 18px' }}>
      ...
      {!loading && error && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, background: C.redBg, border: `1px solid ${C.red}30`, borderRadius: 12, padding: '16px 20px', color: C.red, fontSize: 14 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 14, padding: '20px 24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
              <div style={{ width: 36, height: 36, borderRadius: 9, background: data.prismaDb.connected ? C.greenBg : C.redBg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Activity size={16} color={data.prismaDb.connected ? C.green : C.red} />
              </div>
              <div>
                <div style={{ fontSize: 13, fontWeight: 700, color: C.ink, marginBottom: 4 }}>Prisma DB</div>
                <StatusBadge connected={data.prismaDb.connected} latency={data.prismaDb.latency} />
              </div>
            </div>
            {data.prismaDb.connected && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: C.bgAlt, borderRadius: 8, padding: '8px 12px', fontSize: 12, color: C.mid, fontWeight: 600 }}>
                <HardDrive size={12} /> Taille totale : {data.prismaDb.totalSizeMb} MB
              </div>
            )}
          </div>
          ...
          <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 14, overflow: 'hidden' }}>
            <div style={{ padding: '14px 22px', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, color: C.mid, textTransform: 'uppercase', letterSpacing: '0.07em' }}>
                Tables ({data.prismaDb.tables.length})
              </span>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              ...
              <tbody>
                {data.prismaDb.tables.map((table, i) => (
                  <tr key={table.tableName} onClick={() => setSelectedTable(table.tableName)}
      ...
      {selectedTable && (
        <TableViewerModal tableName={selectedTable} isSuperAdmin={isSuperAdmin} onClose={() => setSelectedTable(null)} />
      )}
