            ...
            <tbody>
              {users.map((user, i) => (
                <tr key={user.id} style={{ borderBottom: i < users.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: user.isActive ? C.greenBg : C.redBg, color: user.isActive ? C.green : C.red, borderRadius: 99, padding: '3px 10px', fontSize: 11, fontWeight: 700 }}>
                      <span style={{ width: 6, height: 6, borderRadius: '50%', background: user.isActive ? C.green : C.red, display: 'inline-block' }} />
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                      ...
                      <button onClick={() => void toggleActive(user)} disabled={actionLoading === user.id}
                        ...
                        style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 10px', border: `1px solid ${user.isActive ? C.red : C.green}30`, borderRadius: 7, background: user.isActive ? C.redBg : C.greenBg, color: user.isActive ? C.red : C.green, cursor: 'pointer', fontSize: 11, fontWeight: 600 }}>
