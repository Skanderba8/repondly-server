...
const ROUTING_RULES: RoutingRule[] = [
  ...
  {
    id: 'admin',
    sources: ['admin.repondly.com'],
    path: '/',
    target: 'admin',
    port: 3006,
    serviceKey: 'app',
    sslDomain: 'admin.repondly.com',
  },
  {
    id: 'bot',
    ...
    <div style={{
      ...
      <div style={{ flex: '1 1 180px', minWidth: 0 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          {rule.path && rule.path !== '/' && (
            <span style={{ fontSize: 11, color: C.blue, fontFamily: 'monospace', fontWeight: 500 }}>
              {rule.path}
            </span>
          )}
        </div>
      ...
      <div style={{ color: C.mid, fontSize: 16, fontWeight: 300, flexShrink: 0, userSelect: 'none' }}>
        →
      </div>

      <div style={{ flex: '1 1 160px', minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: isSpecial ? C.mid : C.ink, fontFamily: 'monospace' }}>
            {rule.target}
            {rule.port && !isSpecial ? <span style={{ color: C.blue }}> :{rule.port}</span> : null}
          </span>
          {isSpecial && (
            <span style={{
  ...
  return (
    <section>
      <div style={{ marginBottom: 14 }}>
        <h2 style={{ fontSize: 15, fontWeight: 700, color: C.ink, margin: 0, marginBottom: 2 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {ROUTING_RULES.map((rule) => (
          <RoutingCard key={rule.id} rule={rule} services={services} ssl={ssl} />
        ))}
      </div>
    </section>
  )
